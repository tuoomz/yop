...
const feeCollected = ethers.utils.parseUnits("1000", CONST.USDC_DECIMALS);
...
describe("FeeCollection [@skip-on-coverage]", async () => {
  ...
  describe("Manage Fees", () => {
    ...
    it("should collect the manage fees", async () => {
      ...
      await expect(
        feeCollection.connect(await impersonate(vault.address)).collectManageFee(ethers.utils.parseUnits("1000", CONST.USDC_DECIMALS))
      )
        .to.emit(feeCollection, "ManageFeesCollected")
        .withArgs(vault.address, CONST.USDC_ADDRESS, creatorFees, protocolFees);
  ...
  describe("Performance Fees", () => {
    ...
    it("should collect the performance fees", async () => {
      ...
      await expect(feeCollection.connect(await impersonate(vault.address)).collectPerformanceFee(strategy.address, feeCollected))
        ...
        .withArgs(strategy.address, CONST.USDC_ADDRESS, proposerFees, developerFees, protocolFees);

      expect(await usdcContract.balanceOf(feeCollection.address)).to.be.equal(feeCollected);
      expect(await feeCollection.connect(developer).feesAvailableForToken(CONST.USDC_ADDRESS)).to.be.equal(developerFees);
    });
  });

  describe("Strategy Reports correct fees", () => {
    const gains = ethers.utils.parseUnits("1000", CONST.USDC_DECIMALS);
  ...
  describe("Claim fees", () => {
    ...
    it("should claim fees available", async () => {
      ...
      await expect(feeCollection.connect(developer).claimAllFees())
        ...
        .withArgs(developer.address, CONST.USDC_ADDRESS, developerFees);
      ...
      expect(await feeCollection.connect(developer).feesAvailableForToken(CONST.USDC_ADDRESS)).to.be.equal(BigNumber.from("0"));
...
async function setUpFeeCollection() {
  ...
  await usdcContract
    .connect(await impersonate(CONST.USDC_WHALE_ADDRESS))
    .transfer(vault.address, ethers.utils.parseUnits("100000", CONST.USDC_DECIMALS));
  await usdcContract
    .connect(await impersonate(CONST.USDC_WHALE_ADDRESS))
    .transfer(strategy.address, ethers.utils.parseUnits("100000", CONST.USDC_DECIMALS));
  await usdcContract
    .connect(await impersonate(vault.address))
    .approve(feeCollection.address, ethers.utils.parseUnits("1000", CONST.USDC_DECIMALS));
