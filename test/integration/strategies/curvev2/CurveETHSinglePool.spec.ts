...
describe("CurveETHSinglePoolStrategy [@skip-on-coverage]", async () => {
  let vault: SingleAssetVault;
  let vaultStrategyDataStore: VaultStrategyDataStore;
  let governance: SignerWithAddress;
  let curveStEthStrategy: CurveETHSinglePool;
  let curveAnkrEthStrategy: CurveETHSinglePool;
  ...
  beforeEach(async () => {
    ...
    const CurveEthStrategyFactory = await ethers.getContractFactory("CurveETHSinglePool");
    curveStEthStrategy = (await CurveEthStrategyFactory.deploy(
      vault.address,
      proposer.address,
      developer.address,
      keeper.address,
      ...
      0
    )) as CurveETHSinglePool;
    curveAnkrEthStrategy = (await CurveEthStrategyFactory.deploy(
      vault.address,
      proposer.address,
      developer.address,
      keeper.address,
    ...
    await vaultStrategyDataStore
      .connect(governance)
      .addStrategy(vault.address, curveStEthStrategy.address, 4500, ethers.constants.Zero, ethers.constants.MaxUint256, 100);
    await vaultStrategyDataStore
      .connect(governance)
      .addStrategy(vault.address, curveAnkrEthStrategy.address, 4500, ethers.constants.Zero, ethers.constants.MaxUint256, 100);
  ...
  describe("Curve stETH happy path", async () => {
    ...
    it("normal operation", async () => {
      ...
      await expect(await curveStEthStrategy.connect(governance).harvest())
        // allocated to the strategy
        .to.emit(wethContract, "Transfer")
        .withArgs(vault.address, curveStEthStrategy.address, allocatedFund)
        // converted to eth
        .to.emit(wethContract, "Withdrawal")
        .withArgs(curveStEthStrategy.address, allocatedFund)
        // fund is added to the pool
        .to.changeEtherBalance(curveStEthPool, allocatedFund);

      await jumpForward(60 * 60 * 24); // 1 day
      await curveStEthStrategy.connect(governance).harvest();
      const estimatedTotal = ethers.utils.formatUnits(await curveStEthStrategy.estimatedTotalAssets(), 18);
    ...
    it("emergency withdraw", async () => {
      await vault.connect(user).deposit(depositAmount, user.address);
      await expect(await curveStEthStrategy.connect(governance).harvest()).to.changeEtherBalance(curveStEthPool, allocatedFund);
      await curveStEthStrategy.connect(governance).setEmergencyExit();
  ...
  describe("Curve ankrETH happy path", async () => {
    const depositAmount = ethers.utils.parseEther("100");
    const allocatedFund = ethers.utils.parseEther("45"); // 90% ratio
    ...
    it("normal operation", async () => {
      ...
      expect(await wethContract.balanceOf(vault.address)).to.equal(depositAmount);
      expect(await vault.balanceOf(user.address)).to.gt(ethers.constants.Zero);
      await expect(await curveAnkrEthStrategy.connect(governance).harvest())
        // allocated to the strategy
        .to.emit(wethContract, "Transfer")
        .withArgs(vault.address, curveAnkrEthStrategy.address, allocatedFund)
        // converted to eth
        .to.emit(wethContract, "Withdrawal")
        .withArgs(curveAnkrEthStrategy.address, allocatedFund)
        // fund is added to the pool
        .to.changeEtherBalance(curveAnkrEthPool, allocatedFund);

      await jumpForward(60 * 60 * 24); // 1 day
      await curveAnkrEthStrategy.connect(governance).harvest();
      const estimatedTotal = ethers.utils.formatUnits(await curveAnkrEthStrategy.estimatedTotalAssets(), 18);
      // it's not going to make any profit as there is fee charged by Curve, so just check it is within certain range.
      expect(parseFloat(estimatedTotal)).to.be.closeTo(parseFloat(ethers.utils.formatUnits(allocatedFund, 18)), 1);
    });
    ...
    it("emergency withdraw", async () => {
      await vault.connect(user).deposit(depositAmount, user.address);
      await expect(await curveAnkrEthStrategy.connect(governance).harvest()).to.changeEtherBalance(curveAnkrEthPool, allocatedFund);
      await curveAnkrEthStrategy.connect(governance).setEmergencyExit();
      const beforeBalance = await wethContract.balanceOf(vault.address);
      await curveAnkrEthStrategy.connect(governance).harvest();
      const afterBalance = await wethContract.balanceOf(vault.address);
      const diff = afterBalance.sub(beforeBalance);
